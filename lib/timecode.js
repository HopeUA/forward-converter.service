...
exports.unixToString = function(time, format){
    ...
    timecode = timecode.replace(/ms/g, Math.floor(date.getUTCMilliseconds() / 10).pad())
                       .replace(/s/g,  date.getUTCSeconds().pad())
                       .replace(/m/g,  date.getUTCMinutes().pad())
                       .replace(/h/g,  date.getUTCHours().pad());
